...
public class EmployeeController {
    ...
    public Response getEmployeeList(){
        ...
        if(personList.size() == 0) {
            ...
            return Response.status(Response.Status.NOT_FOUND).type(MediaType.TEXT_PLAIN).entity(ErrorString.EMPLOYEE_NOT_FOUND_ERROR).encoding("UTF-8").build();
        }
    ...
    public Response getEmployeeReportById(@PathParam("id") UUID id){
        ...
        if(documentList == null){
            ...
            return Response.status(Response.Status.NOT_FOUND).type(MediaType.TEXT_PLAIN).entity(ErrorString.DOCUMENT_NOT_FOUND_ERROR).encoding("UTF-8").build();
        }
